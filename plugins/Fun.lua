			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@pasargad_cute", dl_cb, nil)
					else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '@pasargad_cute', dl_cb, nil)
					else
...
if matches[1] == 'voice' or matches[1] == 'Voice' or matches[1] == 'وی' then
 local text = matches[2]
  ...
  local file = download_to_file(url,'BD-Reborn.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '@pasargad_cute', dl_cb, nil)
   end
end
...
--------------------------------
	if matches[1]:lower() == "photo" or  matches[1]:lower() == "Photo" or matches[1]:lower() == "عک" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@pasargad_cute", dl_cb, nil)
	end
